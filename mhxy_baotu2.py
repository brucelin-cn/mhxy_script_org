...
class Baotu(MhxyScript):
    ...
    def mission(self):
        ...
        if ms is not None:
            ...
            ms = Util.locateCenterOnScreen('resources/baotu/baotu_tingtingwufang.png', region=(
                                           int(frame.left + frameSize[0] / 2), frame.top, int(frameSize[0] / 2), frameSize[1]),confidence=0.85)
            if ms is not None:
                pyautogui.leftClick(ms.x, ms.y)
            else:
